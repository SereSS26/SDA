...
void insertionSortReverse(Film *v, int n)
{
    int i, j;
    Film key;
    for (i = 1; i < n; i++)
    {
        key = v[i];  // Elementul de inserat
        j = i - 1;
        printf("%d\n\n",i);
        // Muta elementele mai mari spre dreapta
        while (j >= 0 && v[j].an > key.an)
        {
            v[j + 1] = v[j];
            j--;
        }
        v[j + 1] = key;  // Inserăm elementul la poziția corectă
    }
}
int main(void)
{   
    ...
    //printf("%d %d\n", comparati, mutari);
    insertionSortReverse(filme,numar_filme);
