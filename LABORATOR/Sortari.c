...
int binary_search(tip_element a[], int item, int st, int dr) {
    while (st <= dr) {
        int mij = st + (dr - st) / 2;
        if (item == a[mij].cheie) {
            return mij + 1; // Inserare după elementul cu cheia egală
        } else if (item > a[mij].cheie) {
            st = mij + 1;
        } else {
            dr = mij - 1;
        }
    }
    return st; // Poziția unde ar trebui inserat elementul
}
...
void insertion_sort2(tip_element a[], int n) {
    int i, j;
    tip_element tmp;

    for (i = 1; i < n; i++) {
        tmp = a[i];
        // Folosim căutarea binară pentru a găsi poziția corectă de inserare
        int poz = binary_search(a, tmp.cheie, 0, i - 1);

        // Deplasăm elementele spre dreapta pentru a face loc lui tmp
        for (j = i; j > poz; j--) {
            a[j] = a[j - 1];
        }
        a[poz] = tmp; // Inserăm elementul în poziția găsită
    }
}
...
void shaker_sort(tip_element a[], int n) {
    int stanga = 0, dreapta = n - 1;
    int i;

    while (stanga < dreapta) {
        // Parcurgere de la stânga la dreapta (bule urcă spre dreapta)
        for (i = stanga; i < dreapta; i++) {
            if (a[i].cheie > a[i + 1].cheie) {
                swap(&a[i], &a[i + 1]);
            }
        }
        dreapta--; // Reducem dreapta, elementul la dreapta este acum sortat

        // Parcurgere de la dreapta la stânga (bule coboară spre stânga)
        for (i = dreapta; i > stanga; i--) {
            if (a[i].cheie < a[i - 1].cheie) {
                swap(&a[i], &a[i - 1]);
            }
        }
        stanga++; // Creștem stanga, elementul din stânga este acum sortat
    }
}
...
void qsortnerec_3way(tip_element a[], int n) {
    int s, d, lt, gt, i;
    tip_element x, temp;
    unsigned char is;
    struct {
